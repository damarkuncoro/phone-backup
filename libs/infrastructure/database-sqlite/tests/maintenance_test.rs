...
use domain::{ConnectionType, Device, DeviceId, FileEntry, FileId, SnapshotId};
use phone_backup_adapter_database_sqlite::SqliteRepository;
use ports::{
    ContactRepositoryPort, DeviceRepositoryPort, FileRepositoryPort, MaintenanceRepositoryPort,
...
fn test_maintenance_referenced_hashes() {
    ...
    repo.save_physical_object(&chunk_id, "obj-hash-1", "uuid-1", 10, "none", 0)
        .unwrap();
    repo.save_file_chunk(&FileId("f2".to_string()), &chunk_id, 0, 10, 0)
        .unwrap();
...
fn test_prune_orphans() {
    ...
    repo.save_device(&domain::Device {
        ...
        storage_used_bytes: 0,
        storage_free_bytes: 0,
        connection_type: domain::ConnectionType::Usb,
    })
    .unwrap();
    ...
    repo.create_snapshot(&domain::Snapshot {
        ...
        started_at: Utc::now(),
        finished_at: None,
        status: domain::SnapshotStatus::Pending,
        total_files: 0,
        total_bytes: 0,
        deduped_bytes: 0,
    })
...
fn test_database_backup() {
    ...
    repo.save_device(&domain::Device {
        id: domain::DeviceId::new("dev-backup"),
        manufacturer: "A".to_string(),
        model: "B".to_string(),
        serial: "C".to_string(),
