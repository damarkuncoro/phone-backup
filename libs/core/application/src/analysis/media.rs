    ...
    fn extract_image_info(data: &[u8]) -> Option<MediaInfo> {
        ...
        if let Ok(exif_data) = exif::Reader::new().read_from_container(&mut reader) {
            ...
            if let (Some(lat_field), Some(lat_ref), Some(lon_field), Some(lon_ref)) = (
                ...
                info.latitude = Self::parse_gps_coordinate(
                    lat_field,
                    lat_ref.display_value().to_string().contains('S'),
                );
                info.longitude = Self::parse_gps_coordinate(
                    lon_field,
                    lon_ref.display_value().to_string().contains('W'),
                );
            }
