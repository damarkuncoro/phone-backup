use anyhow::Result;
use domain::{Manifest, ManifestFile, ManifestChunk, Snapshot};
...
impl<'a, R: RepositoryPort, T: StoragePort> ManifestManager<'a, R, T> {
    ...
    pub fn create_and_store_manifest(&self, snapshot: &Snapshot) -> Result<()> {
        info!("Creating immutable manifest for snapshot: {}", snapshot.id.0);
        ...
        for file in files {
            ...
            let chunks = chunks_data.into_iter()
        ...
        self.storage.write(&manifest_path, &mut std::io::Cursor::new(manifest_json))?;
