...
impl ExpertChunker {
    ...
    pub fn create_stream<R: Read + 'static>(
        ...
        match method {
            ChunkingMethod::FixedSize => Box::new(strategies::fixed::FixedChunker::new(reader, config)),
            ChunkingMethod::FastCDC => Box::new(strategies::fastcdc::FastCdcChunker::new(reader, config)),
            ChunkingMethod::FullFile => Box::new(strategies::fullfile::FullFileChunker::new(reader, config)),
        }
    }

    /// Helper for quick static data chunking.
    pub fn chunk_data(data: &[u8], method: ChunkingMethod, config: ChunkConfig) -> Result<Vec<(Chunk, Vec<u8>)>> {
