    ...
    pub fn parse_sms(output: &str) -> Vec<Sms> {
        ...
        for line in output.lines() {
            if let (Some(address), Some(body), Some(date_str)) = (
                ...
                let type_code = ParserUtils::extract_value(line, "type").and_then(|s| s.parse().ok()).unwrap_or(1);
                messages.push(Sms {
                    address,
                    body: body.replace("\\n", "\n"),
        ...
        for line in output.lines() {
            if let (Some(number), Some(date_str), Some(duration_str)) = (
                ...
                logs.push(CallLog {
                    ...
                    type_code: ParserUtils::extract_value(line, "type").and_then(|s| s.parse().ok()).unwrap_or(1),
