    ...
    pub fn parse_mediastore(device_id: &DeviceId, output: &str) -> Vec<FileEntry> {
        output.lines()
            ...
            .filter_map(|line| {
                ...
                let size = ParserUtils::extract_value(line, "_size").and_then(|s| s.parse().ok()).unwrap_or(0);
                let mtime = ParserUtils::extract_value(line, "date_modified").and_then(|s| s.parse().ok()).unwrap_or(0);
                ...
                let height = ParserUtils::extract_value(line, "height").and_then(|s| s.parse().ok());
                let taken_at_ms = ParserUtils::extract_value(line, "datetaken").and_then(|s| s.parse::<i64>().ok());
                let lat = ParserUtils::extract_value(line, "latitude").and_then(|s| s.parse().ok());
                let lon = ParserUtils::extract_value(line, "longitude").and_then(|s| s.parse().ok());
