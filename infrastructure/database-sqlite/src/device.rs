use crate::mappers::map_row_to_device;
use domain::{Device, DeviceId};
use rusqlite::{params, Connection};

pub fn save_device(conn: &Connection, device: &Device) -> anyhow::Result<()> {
    conn.execute(
...
pub fn list_devices(conn: &Connection) -> anyhow::Result<Vec<Device>> {
    ...
    let device_iter = stmt.query_map([], map_row_to_device)?;
...
pub fn get_device(_conn: &Connection, _id: &DeviceId) -> anyhow::Result<Option<Device>> {
    Ok(None)
}
