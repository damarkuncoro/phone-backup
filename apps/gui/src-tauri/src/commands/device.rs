...
pub async fn scan_device(
    ...
    device_id: String,
) -> Result<Vec<domain::FileEntry>, String> {
...
pub async fn delete_device_file(
    ...
    state
        .engine
        .delete_remote(&id, &path)
        .map_err(|e| e.to_string())
}

#[tauri::command(rename_all = "snake_case")]
pub async fn rename_device_file(
    state: State<'_, AppState>,
    device_id: String,
    old_path: String,
    new_path: String,
...
pub async fn copy_device_file(
    ...
    state
        .engine
        .copy_remote(&id, &source, &target)
        .map_err(|e| e.to_string())
}

#[tauri::command(rename_all = "snake_case")]
pub async fn upload_to_device(
    state: State<'_, AppState>,
    device_id: String,
    local_path: String,
    remote_path: String,
    ...
    state
        .engine
        .upload_file(&id, &local_path, &remote_path)
        .map_err(|e| e.to_string())
}

#[tauri::command(rename_all = "snake_case")]
pub async fn download_from_device(
    state: State<'_, AppState>,
    device_id: String,
    remote_path: String,
    local_path: String,
    ...
    state
        .engine
        .download_file(&id, &remote_path, &local_path)
        .map_err(|e| e.to_string())
}

#[tauri::command(rename_all = "snake_case")]
pub async fn calculate_device_file_hash(
    state: State<'_, AppState>,
    device_id: String,
    path: String,
    ...
    state
        .engine
        .calculate_hash(&id, &path)
        .map_err(|e| e.to_string())
}

#[tauri::command(rename_all = "snake_case")]
pub async fn get_live_apps(
    state: State<'_, AppState>,
    device_id: String,
) -> Result<Vec<domain::AppInfo>, String> {
    let id = DeviceId::new(device_id);
