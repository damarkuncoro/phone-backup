...
) -> Result<application::StorageStats, String> {
...
pub async fn start_backup(
    ...
    let policy =
        include_files.map(|paths| domain::BackupPolicy::builder().include_many(paths).build());
    state
        .engine
        .perform_backup(&id, domain::EncryptionMode::None, policy)
...
#[tauri::command(rename_all = "snake_case")]
pub async fn get_snapshot_apps(
    state: State<'_, AppState>,
    snapshot_id: String,
...
pub async fn get_contact_diff(
    ...
    state
        .engine
        .get_contact_diff(&old_id, &new_id)
        .map_err(|e| e.to_string())
}
