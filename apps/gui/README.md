...
- **`permissions/` & `gen/schemas/`**: Security capability declarations (`autogenerated.toml`, `acl-manifests.json`) enforcing access control for commands.
...
- **`components/`**: Atomic UI building blocks using Native Web Components:
    - `browser/`: Specialized sub-views for the data explorer (FileListView, AndroidDataView, AppListView).
    - `DeviceItem.js`, `SnapshotList.js`, etc.: Encapsulated domain-specific UI elements.

## 🚀 Key Features

- **Live Phone File Manager**: Interactive file management (browse, live search, `download_from_device`, upload, rename, copy, delete, and SHA-256 hash calculation) directly on connected devices.
- **Visual Snapshot Diffing UI**: Interactive diff view highlighting **New**, **Modified**, **Deleted**, and **Unchanged** items between snapshot iterations.
- **Installed Apps Explorer**: Inspect live and snapshotted Android applications, package names, and app metadata.
- **Tauri Security & ACL Manifests**: Strict capabilities configuration restricting IPC command access to authorized endpoints.
